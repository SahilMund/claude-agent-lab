...
# tree-sitter-languages is unmaintained and has no wheels for current Python
# versions (fails to install on 3.13). tree-sitter-language-pack is the
# actively maintained fork with the same get_language()/get_parser() API —
# a drop-in swap, not a design change.
...
def _parse_with_treesitter(source: str, filepath: str, language_name: str) -> list[ParsedChunk]:
  ...
  # Tree-sitter node offsets (start_byte/end_byte) are byte offsets into the
  # UTF-8 *encoding* of the source, not character offsets into the Python
  # str. Slicing the str directly with them silently corrupts every chunk
  # (name and content both) as soon as a multi-byte character — an em dash,
  # an arrow, a smart quote, all over this codebase's own docstrings —
  # appears anywhere earlier in the file. Keep the encoded bytes around and
  # slice those instead; decode only the final result.
