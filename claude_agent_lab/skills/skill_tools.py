...
from claude_agent_lab.skills.registry import SkillRegistry, SkillNotFoundError
...
# claude_agent_lab package directory.
#
# State transitions:
...
def get_registry() -> SkillRegistry:
   ...
   skills_dir resolution order:
     1. Read "skills.skills_dir" from config.yaml  →  e.g. ".agent_lab/skills"
     2. Resolve relative to Path.cwd()             →  the target project root
        (claude_agent_lab is invoked from the project being analysed, so cwd()
         IS the target project, not the claude_agent_lab package itself)

   Example:
     User runs claude_agent_lab from /home/user/my_project/
     config.yaml says skills_dir: .agent_lab/skills
     → skills are loaded from /home/user/my_project/.agent_lab/skills/
   """
   global _registry
   if _registry is None:
       skills_dir = Path.cwd() / config.get("skills", {}).get("skills_dir", ".agent_lab/skills")
